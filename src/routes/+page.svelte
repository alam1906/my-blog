<script lang="ts">
	...
	const categories = ['All', 'Rubik', 'Programming', 'Magic', 'Chess', 'Daily', 'Review', 'Other'];
...
<div in:fade={{ duration: 300 }}>
		...
		<div class=" my-10 flex-wrap space-y-2 space-x-2 md:flex md:justify-center md:space-y-0">
			{#each categories as category}
				<button
					...
					class:bg-teal-500={category === selectedCategory}>{category}</button
				>
			{/each}
		</div>
					...
					{#each visibleBlogs as blog}
						<a href={slugify(`/${blog.title}`.toLowerCase())}
							><div class="cursor-pointer shadow-lg">
								<div class="mb-1 aspect-video bg-gray-200"></div>
								<div class="mx-3 my-3 flex flex-col">
									<div class="font-normal text-teal-400">{`#${blog.category}`}</div>
									<div class="text-xl font-semibold">{blog.title}</div>
									<div class="my-2 line-clamp-3 text-sm text-gray-500">{blog.description}</div>
									<div class="mb-2 flex cursor-pointer items-center space-x-1">
										<div class="font-semibold text-teal-500">Read More</div>
										<MoveRight class="mt-1 text-teal-500" />
									</div>
								</div>
							</div></a
						>
	...
	{#if visibleCount < filterBlog().length}
		<div class="flex w-full justify-center">
			<button onclick={loadMore} class="my-8 rounded-xl bg-teal-500 px-7 py-2 text-center"
				>Load More</button
			>
		</div>
	{:else}
